...
deconvolve <- function(norm_data, gam = 0.95, lambda = 1, constraint = T, estimate_calcium = T, var = "gam_detrended", ESP = 0.0001,
  ...
  if(dim(peaks_data)[1] != 0) {
    ...
    peaks_data <- unique(peaks_data[,c("Cell_id", "spike_frame", "spike_stimulus",
                                       "spike_smooth_z", "Mean_Grey", "gam_detrended", "Max_peak_frame", "max_peak_smooth_z", "spike_smooth_Diff", "lag_stim", "Time_frame_stim", "Prev_stim" )])



  peaks_data <- peaks_data[,lead_spike_smooth_Diff_1 := dplyr::lead(spike_smooth_Diff, 1, default = 0), by = Cell_id]
