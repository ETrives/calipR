...
print("patdec ok" )
  ...
  norm <- norm_df(back, var = "back", width = 20)
...
downstream_analysis_bis <- function(data, moving_thresh = 0.1, outlier_thresh = 2, mean_width = 20, DPA_width = 10, CN_DPA_width = 20,
                                    mean_width_diff = 10, method = "gam", norm_var = "gam",
                                    norm_width = 10, lambda = 100,
                                    gam = 0.97, constraint = T, threshold = 3,
                                    deconvolve_var = "gam_detrended", borders_range = 50,
  ...
  if(one_cell == TRUE){
    ...
    if(pattern_matching == TRUE){
      ...
      back <- patDetectR(data, windows, steps, new_len = 30, posBank,
                         negBank, Var = "Mean_Grey")
