...
clean_data <- function(data, moving_threshold, outlier_threshold ,mean_width, DPA_width, mean_width_diff, mean_width_second){
  ...
  cell_split <- lapply(cell_split, function(x) data.table::setDT(x)[, local_mean := MazamaRollUtils::roll_mean(x$Mean_Grey, width = mean_width)])
  ...
  cell_split <- lapply(cell_split, function(x) data.table::setDT(x)[, smooth_DPA := MazamaRollUtils::roll_mean(x$DPA, width = 10)])


  cell_split <- lapply(cell_split, function(x) x[, Mean_Grey_wo_peaks := replace(Mean_Grey, smooth_DPA > quantile_speed(smooth_DPA, probs = .2)
